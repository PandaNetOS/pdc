...
pub struct PeerRepoImpl {
    ...
    /// 閸忋劌鐪?peer addr -> PeerInfo閿涘牐娉?infohash 閸樺鍣搁敍?（热点路径，分片锁）
    global: ShardedHashMap<SocketAddr, PeerInfo>,
    /// peer addr -> 閸戣櫣骞囬崷銊ユ憿娴?infohash 娑?（分片锁）
    infohash_refs: ShardedHashMap<SocketAddr, FxHashSet<Infohash>>,
    storage: Arc<Storage>,
...
impl PeerRepoImpl {
    ...
    pub fn with_tier_config(
        ...
        Self {
            by_infohash: ShardedHashMap::new(16),
            global: ShardedHashMap::new(16),
    ...
    /// 鏈皟鐢ㄦ椂锛堝鍗曞厓娴嬭瘯锛夛紝鏈湴鍐欏叆涓嶈Е鍙戜紶鎾紝repo 琛屼负瀹屽叏涓嶅彉銆?
    ...
    /// merkle/gossip 鏈敞鍏ユ椂鐩存帴璺宠繃锛屼笉 panic銆?
    #[inline]
    ...
    pub fn ensure_loaded(&self) {
        if self.global.is_empty() {
            info!("[federation] PeerRepo cache 涓虹┖锛岄噸鏂粠 SQLite 鍔犺浇...");
            match self.storage.load_peers() {
                Ok(rows) => {
                    // 锁顺序：global → by_infohash → infohash_refs
                    let mut gl = self.global.write_all();
                    let mut bi = self.by_infohash.write_all();
                    ...
                    for row in &rows {
                        ...
                        gl.insert(addr, peer);
                        // by_infohash: entry-or-default
                        if let Some(set) = bi.get_mut(&row.infohash) {
                            set.insert(addr);
                        } else {
                            let mut s = FxHashSet::default();
                            s.insert(addr);
                            bi.insert(row.infohash, s);
                        }
                        // infohash_refs: entry-or-default
                        if let Some(refs) = ir.get_mut(&addr) {
                            refs.insert(row.infohash);
                        } else {
                            let mut s = FxHashSet::default();
                            s.insert(row.infohash);
                            ir.insert(addr, s);
                        }
                        count += 1;
                    }
    ...
    pub(crate) fn add_peers_sync_internal(
        ...
        for (infohash, peer) in items {
            ...
            let is_new_assoc = if let Some(set) = bi.get_mut(infohash) {
                ...
                let mut s = FxHashSet::default();
                s.insert(addr);
                bi.insert(*infohash, s);
                true
            };
            if let Some(refs) = ir.get_mut(&addr) {
                refs.insert(*infohash);
            } else {
                let mut s = FxHashSet::default();
                s.insert(*infohash);
                ir.insert(addr, s);
        ...
        // 缂撳啿鍖烘弧 100 鏉℃椂鑷姩 flush锛堜笌鍘熻涓轰竴鑷达級
        if buffer.len() >= 100 {
    ...
    /// 鑱旈偊鍚屾鍏ョ珯锛坅pply_peer_sync锛夎皟鐢ㄦ湰鏂规硶锛岄伩鍏?Merkle 閲嶅鏇存柊涓?Gossip 鍥炵幆銆?
    #[allow(dead_code)]
    ...
    /// 鎶婃柊 peer 鍒楄〃鏋勫缓鎴?merkle/gossip 鏉＄洰骞朵紶鎾锛堜竴娆¤閿侊級
    ...
    /// 鏈湴鍐欏叆璺緞锛氭柊 peer 鏇存柊 Merkle + 鎻愪氦 Gossip銆?
    /// 鑱旈偊鍚屾鍏ョ珯锛坅pply_peer_sync锛夎鏀圭敤 add_peers_only_internal锛岄伩鍏嶅洖鐜€?
    ...
    pub fn get_peers_sync(&self, infohash: &Infohash, limit: usize) -> Vec<PeerInfo> {
        // 先读 by_infohash 拿到 addrs 列表，再逐个 global.get 查 PeerInfo
        let addrs: Vec<SocketAddr> = self
            .by_infohash
            .get(infohash)
            .map(|set| set.iter().cloned().collect())
            .unwrap_or_default();
        let mut result: Vec<PeerInfo> = addrs.iter().filter_map(|a| self.global.get(a)).collect();
        result.sort_by(|a, b| {
            b.priority_score
    ...
    pub fn stats(&self) -> (usize, usize) {
        (self.by_infohash.len(), self.global.len())
    }

    pub fn len_for_infohash(&self, infohash: &Infohash) -> usize {
        self.by_infohash.get(infohash).map(|s| s.len()).unwrap_or(0)
    }
    ...
    pub fn infohashes(&self) -> Vec<Infohash> {
        let bi = self.by_infohash.read_all();
        bi.keys().cloned().collect()
    }
    ...
    /// 閸忋劑鍣烘穱婵嗙摠閸?SQLite
    /// 濞夈劍鍓伴敍姘枎閻戭厼鍨界€规氨绮烘稉鈧悽?intelligence 鐏炲倻娈?TierSystem 鐠愮喕鐭楅敍宀冪箹闁插苯鍙忛柌蹇庣箽鐎涙ɑ澧嶉張?peer
    pub async fn save_all(&self) -> anyhow::Result<()> {
        // 先 read_all() by_infohash 拿到 (infohash, addr) 列表，再逐个 global.get
        let pairs: Vec<(Infohash, SocketAddr)> = {
            let bi = self.by_infohash.read_all();
            bi.iter()
                .flat_map(|(ih, addrs)| addrs.iter().map(move |a| (*ih, *a)))
                .collect()
        };
        let batch = {
            let mut batch = Vec::with_capacity(pairs.len());
            for (infohash, addr) in pairs {
                if let Some(peer) = self.global.get(&addr) {
        ...
        if let Some(wq) = &self.write_queue {
            ...
            // 鍚屾妯″紡�氫繚鐣欏師 spawn_blocking 閫昏緫
            let storage = self.storage.clone();
...
impl PeerRepository for PeerRepoImpl {
    ...
    async fn remove_peer(&self, infohash: &Infohash, addr: &SocketAddr) {
        ...
        if let Some(set) = bi.get_mut(infohash) {
            set.remove(addr);
    ...
    async fn update_scores_batch(&self, scores: &[(SocketAddr, f64)]) {
        ...
        for (addr, score) in scores {
            if let Some(peer) = gl.get_mut(addr) {
                peer.priority_score = *score;
                dirty.insert(*addr);
            }
        }
