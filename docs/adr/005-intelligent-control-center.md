...
- **验证式控制**：控制闭环升级为 Observe-Decide-Act-Verify，指令版本化 + snapshot 回读校验；未生效重试、连续失败标记失控并上报 pk（详见 08 文档 5.1–5.5）。
- **结构性强制**：资源配额以账本租约实现（模块无独立并发计数）、周期任务统一走 TaskScheduler 收口 API，控制从约定变为结构约束。
- **冲突仲裁**：指令优先级 pk 意图 > ICC 策略 > 本地 TOML > 默认值，避免配置与策略打架。
- **Fail-open 分级**：ICC 失效时模块退回默认调度（等同现状），且仅限软策略降级，硬约束（热路径/数据/评分唯一性）永不降级。
...
- [ ] 指令版本化 + snapshot 回读校验生效：下发后版本收敛一致，连续未生效可标记模块失控并触发告警。
- [ ] 全部单元测试通过，现有行为无回归。
...
| 2026-09-10 | 1.1 | 补充验证式控制契约（Observe-Decide-Act-Verify）与失控处置，缓解措施/验证标准同步 | 项目维护者 |
